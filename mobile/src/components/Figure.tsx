...
/**
 * The anchor's width, declared rather than inherited.
 *
 * **This is what broke it the first time.** The anchor took
 * `alignSelf: 'stretch'` inside a `Pressable` that shrinks to fit its content,
 * so the anchor's width came from the `Pressable`, whose width came from the
 * anchor — and Yoga settled that circle at **zero**. Every absolutely-filled
 * piece then filled a zero-width box and the entire amount drew nothing at all.
 *
 * It is the widest figure the keypad allows: seven digits, two separators, the
 * sign and the mark. 336.31 of the 339 the panel has — see `fits.py`, it is
 * tight — so it never has to stretch to anything and nothing has to measure it.
 */
const FIGURE_W = 2 * (MARK_W + GAP) + 7 * DIGIT_W + 2 * PUNCT_W
...
const s = StyleSheet.create({
  /*
   * An explicit width, and centred by itself. Never `stretch`: its parent is a
   * `Pressable` that shrinks to fit, so stretching asks the anchor how wide it
   * is in order to answer how wide it is, and Yoga answers zero.
   */
  anchor: { width: FIGURE_W, alignSelf: 'center' },
  /* In flow, so the anchor takes exactly the height a 60pt line has always had. */
  gauge: { opacity: 0 },
  /*
